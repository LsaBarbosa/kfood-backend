...
@Service
@ConditionalOnBean(StoreCheckoutRulesValidator.class)
public class DefaultCheckoutCriticalValidationService implements CheckoutCriticalValidationService {
  ...
  @Override
  public void revalidate(Store store, CheckoutQuoteSnapshot quoteSnapshot) {
    storeCheckoutRulesValidator.ensureStoreOperational(store);
    storeCheckoutRulesValidator.ensureStoreWithinBusinessHours(store);
    var customer =
        customerRepository
            .findByIdAndStoreId(quoteSnapshot.customerId(), store.getId())
            .orElseThrow(
                () ->
    ...
    var products =
        catalogProductRepository.findAllByStoreIdAndIdIn(
            store.getId(), quoteSnapshot.items().stream().map(item -> item.productId()).toList());
    var productsById =
        products.stream()
